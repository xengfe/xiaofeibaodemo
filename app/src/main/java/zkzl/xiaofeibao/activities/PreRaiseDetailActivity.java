...
@ContentView(R.layout.activity_pre_raise_detail)
public class PreRaiseDetailActivity extends BaseActivity implements ShareDialog.ShareListener,
    ...
    @ViewInject(R.id.pre_header_image_view)
    private ImageView headImageView;
    @ViewInject(R.id.pre_time_tv)
    private TextView rasieDateTextView;
    @ViewInject(R.id.pre_header_name)
    private TextView raiseNameTextView;

    @ViewInject(R.id.pre_raise_progressbar)
    private ProgressBar progressBar;
    @ViewInject(R.id.pre_raise_wactcher_num)
    private TextView watcherNumTextView;

    @ViewInject(R.id.pre_raise_left_day)
    private TextView raiseLeftDayTextView;


    @ViewInject(R.id.pre_raise_plan)
    private Button raisePalnButton;
    @ViewInject(R.id.pre_raise_promote)
    private Button raisePromoteButton;
    @ViewInject(R.id.pre_raise_earn)
    private Button rasieEarnButton;
    @ViewInject(R.id.pre_raise_security)
    private Button raiseSecurityButton;

    @ViewInject(R.id.pre_raise_focus)
...
//        sharePopuWin.showAtLocation(findViewById(R.id.pre_raise_main), Gravity.CENTER, 0, 0);
    ...
    public void share(View v) {
        switch (v.getId()) {
            case R.id.share_qq:
                ...
                qq.setPlatformActionListener(PreRaiseDetailActivity.this); // 设置分享事件回调
                qq.share(spQQ);
                break;
            case R.id.share_qzion:
                ...
                zone.setPlatformActionListener(PreRaiseDetailActivity.this);
                zone.share(spZone);
                break;
            case R.id.share_wechat:
                ...
                wechat.setPlatformActionListener(PreRaiseDetailActivity.this); // 设置分享事件回调
                wechat.share(sp);
                break;
            case R.id.share_wechat_fri:
                ...
                wechatMoments.setPlatformActionListener(PreRaiseDetailActivity.this); // 设置分享事件回调
    ...
    @Event(R.id.pre_call_button)
    private void callButtonClick(View view) {
//        CallPopuWin callPopuWin = new CallPopuWin(this,"刘志伟","15010183136",this);
//        callPopuWin.showAtLocation(findViewById(R.id.pre_raise_main), Gravity.CENTER, 0, 0);
